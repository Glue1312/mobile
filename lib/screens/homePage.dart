...
class _MyHomePageState extends State<MyHomePage> {
          ...
          child: SingleChildScrollView(
            child: Container(
              ...
              child: Column(children: [
                ...
                Text('Atau pilih preferensi di bawah',
                  ...
                  child: ElevatedButton(
                      ...
                      onPressed: () async {
                        ...
                        (selecteds[0] == false || selecteds[1] == false)
                            ? showSnackBarMessage(
                                'Pilih preferensi apa pun untuk mencari')
                            : Navigator.push(
                                context,
                                MaterialPageRoute(
                                    builder: (context) =>
                      ...
                      child: LetterPokemon(text: 'Mencari', size: fontsize * 0.0012)),
                ),
                  const SizedBox(height: 10),
              ]),
            ),
  ...
  pokemonSearch(context, appStore) async {
    if (await isValidInput() == true) {
      ...
      if (temp[0].name == '') {
        await showSnackBarMessage('Ketikkan nama Pokémon dengan benar');
      } else {
  ...
  isValidInput() async {
    if (controller.text == '') {
      await showSnackBarMessage(
          'Masukkan nama Pokémon yang ingin Anda tanyakan');
      return false;
    } else {
      if (controller.text.contains(RegExp(r'[A-Z]'))) {
        await showSnackBarMessage('Ketik hanya huruf kecil saja');
        return false;
      }
      if (controller.text.contains(RegExp(r'[0-9]'))) {
        await showSnackBarMessage('Ketik hanya huruf');
        return false;
      }
      return true;
    }
